 ...
 */
function theme_debonairson_css_tree_post_processor($tree, $theme) {
 ...
 * @return string
 */
function theme_debonairson_get_main_scss_content($theme) {
    global $CFG;
    ...
    $iterator = new DirectoryIterator($CFG->dirroot . '/theme/debonairson/scss/preset/');
    $presetisset = '';
    foreach ($iterator as $pfile) {
        if (!$pfile->isDot()) {
            $presetname = substr($pfile, 0, strlen($pfile) - 5); // Name - '.scss'.
            if ($filename == $presetname) {
                $scss .= file_get_contents($CFG->dirroot . '/theme/debonairson/scss/preset/' . $pfile);
                $presetisset = true;
            }
        }
    }
    if (!$presetisset) {
        $filename .= '.scss';
        if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_debonairson', 'preset', 0, '/', $filename))) {
    ...
    $scss .= file_get_contents($CFG->dirroot . '/theme/debonairson/scss/debonairson_variables.scss');
    ...
    $scss .= file_get_contents($CFG->dirroot . '/theme/debonairson/scss/styles.scss');
 ...
 * @return array
 */
function theme_debonairson_get_pre_scss($theme) {
    global $CFG, $PAGE;
    ...
    'blockwidthdebonairson' => ['blockwidthdebonairson'],
...
function theme_debonairson_get_extra_scss($theme) {
